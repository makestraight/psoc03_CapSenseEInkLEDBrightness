...
static void process_touch(void)
{
    ...
    static uint16_t slider_pos_former = 0;
    ...
    if((0u == slider_touched) && (0u != slider_touched_prev))
    {
    	...
    	if(slider_pos > slider_pos_former){
            eink_cmd_data = EINK_RIGHT;
    	}else if(slider_pos < slider_pos_former){
            eink_cmd_data = EINK_LEFT;
    	}

    	// only if in menu page
    	if(!led_brightness){
            send_eink_command = true;
    	}
    }
    ...
    /* Send command to update LED state if required */
    if(send_eink_command)
    {
        xQueueSendToBack(eink_command_q, &eink_cmd_data, 0u);
    }
